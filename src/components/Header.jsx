...
function Header() {
    ...
    return (
        <header className="fixed top-0 left-0 m-auto w-full max-w-screen-2xl bg-white ring-1 ring-slate-900/5 z-10 xl:gap-x-10">
            <div className="px-4 flexBetween py-3 max-xs:px-2">
                ...
                <div className="flexBetween sm:gap-x-6 bold-16">
                    {!menuOpened ? (
                        <MdMenu
                            className="md:hidden cursor-pointer hover:text-secondary mr-2 p-1 right-1 ring-slate-900/30 h-8 w-8 rounded-full"
                            onClick={toggleMenu}
                        />
                    ) : (
                        <MdClose
